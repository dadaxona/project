...
class KlentController2 extends Controller
{
    public function tavar_tip(Request $request)
    {
        ...
        if($query != '')
        {
        $data = Adress::where('name', 'like', '%'.$query.'%')->get();
        }
        else
        {
        $data = Adress::get();
    ...
    public function nolas(Request $request)
    {
        if($request->ajax())
        {
        $output = '';
        ...
        if($query != '')
        {
        $data = Ichkitavar::where('name', 'like', '%'.$query.'%')->where('hajm', "<", 1)->get();
        }
        else
        {
        $data = Ichkitavar::where('hajm', "<", 1)->get();
        }
        $total_row = $data->count();
        if($total_row > 0)
        {
            foreach($data as $row)
            {
                ...
                <td  style="background-color: rgb(237, 0, 0); color: white;">'.$row->hajm.'</td>
                <td>'.$row->summa.'</td>
                <td>'.$row->summa2.'</td>
                <td>'.$row->summa3.'</td>
                <td>'.$row->kod.'</td>
   ...
   <tr style="border-bottom: 1px solid;">
    ...
    public function tavar(Request $request)
    {
        ...
        $output = '';
        $query = $request->get('id');
        $iiid = Adress::find($query);
        $data = Ichkitavar::where('adress', $iiid->adress)->get();
        $get = Ichkitavar::all();
        $total_row = $data->count();
        if($total_row > 0)
        {
    ...
    public function datasearche(Request $request)
    {
        if($request->ajax()){
            $output = '';
            $dateearche = $request->get("dateearche");
            $date2earche = $request->get("date2earche");
            if($date2earche){
                $data = Ichkitavar::whereBetween('updated_at', [$dateearche, $date2earche])->get();
                $total_row = $data->count();
                if($total_row > 0)
                {
                    foreach($data as $row)
